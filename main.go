...
func main() {
	go func() {
		_,_,_,PrName,_:=rConfig.Read()
		ipv6,_:=scangetway.Reipv()
		b,_,_:=GetDescribeDomainRecords.GetAna(PrName,ipv6)
		if b!=0{
			fmt.Println("没有找到",PrName,"的主机记录","即将创建记录，并添加记录值:",ipv6)
			MidRecordID.SetAnalysis(PrName,ipv6)
		}
		for {
			fmt.Println("睡眠中.....")
			time.Sleep(15*time.Second)
			ipv6buf,_:=scangetway.Reipv()
			if ipv6!=ipv6buf {
				ipv6=ipv6buf
				b,_,_:=GetDescribeDomainRecords.GetAna(PrName,ipv6)
				if b!=0{
			...
			fmt.Println("存在",PrName,"的主机记录","当前记录值:",ipv6)
		}
	}()
